...
const CampaignsTable: React.FC = () => {
  ...
  return (
    <Box>
      ...
      <TableContainer component={Paper} sx={{ margin: '20px auto', maxWidth: '900px', pl:2, pr:2,borderRadius: '10px', overflow: 'x'  }}>
        ...
        <Table>
          <TableHead >
            <TableRow sx={{bgcolor: '#F3F3F3', borderRadius:'10px',}}>
              ...
              <TableCell sx={{color: '#495057', textAlign:"center" }}>DELIVER</TableCell>
          ...
          <TableRow>&nbsp;</TableRow>
            {campaigns.map((campaign) => (
              <>
              <TableRow key={campaign.id} sx={{borderRadius:'10px', border: '2px solid #ECEEF6', }} >
              
                <TableCell>
                  <Button sx={{color:"#0057D9", bgcolor:"#F2F7FF",  fontSize:"12px", }} >{campaign.id}</Button>
                  <Typography sx={{fontWeight:"bold", fontSize:"12px", pt:0.25, pb:0.25 }}>{campaign.name}</Typography>
                  <Typography sx={{color:"#626262", fontSize:"12px", }}>Published on {campaign.publishedOn}</Typography>
                  </TableCell>
                <TableCell
                  sx={{
                  }}
                ><Button sx={{
                  color: campaign.status === 'Expired' ? '#F83738' : '#52B141',
                  bgcolor: campaign.status === 'Expired' ? '#F8DDDD' : '#E6F5E3',
                  fontSize: '12px',
                ...
                <TableCell sx={{textAlign:"center"}}>
                  <IconButton onClick={() => console.log('Edit clicked', campaign.id)}>
                    <EditIcon sx={{fontSize:'15px',color: '#6A7075', bgcolor: '#EFEFEF', p:0.7, borderRadius:'4.5px' }} />
                  </IconButton>
                  <IconButton onClick={() => console.log('Edit clicked', campaign.id)}>
                    <PauseIcon sx={{fontSize:'15px',color: '#6A7075', bgcolor: '#EFEFEF', p:0.7, borderRadius:'4.5px' }} />
                  </IconButton>
                  <IconButton onClick={() => console.log('Edit clicked', campaign.id)}>
                    <ContentCopyIcon sx={{fontSize:'15px',color: '#6A7075', bgcolor: '#EFEFEF', p:0.7, borderRadius:'4.5px' }} />
                  </IconButton>
                  <IconButton sx={{m:'0px',}} onClick={() => console.log('Delete clicked', campaign.id)}>
                    <DeleteIcon sx={{ fontSize:'16px', color: 'white', bgcolor: '#F83738', p:0.5, borderRadius:'4.5px' }} />
                  </IconButton>
                </TableCell>
              </TableRow>
              <TableRow>&nbsp;</TableRow>
